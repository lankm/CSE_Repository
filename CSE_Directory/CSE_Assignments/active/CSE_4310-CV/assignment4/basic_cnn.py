...
class BasicCNN(L.LightningModule):
    def __init__(self, num_classes=10):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(3,128,3, padding=1),   # 1 input channel (grayscale) to 6 output channels. 3x3 kernal
            nn.ReLU(),
            nn.MaxPool2d(2),

            nn.Conv2d(128,64,5, padding=2),   # 1 input channel (grayscale) to 6 output channels. 3x3 kernal
            nn.ReLU(),
            nn.MaxPool2d(2),

            nn.Conv2d(64,32,3, padding=1),   # 1 input channel (grayscale) to 6 output channels. 3x3 kernal
            nn.ReLU(),
            nn.MaxPool2d(2),
        )

        self.estimator = nn.Sequential(
            nn.Linear(8 * 8 * 32, 512), # input
            nn.ReLU(),
            nn.Linear(512, 128), # outputs of previous match current inputs
            nn.ReLU(),
...
def main():
    ...
    torch.save(model.state_dict(), 'model_weights/basic_cnn')
